...
/**
 * Криптографик санамсаргүй тоо, `[0, 1)` завсарт.
 *
 * ⚠️ `Math.random` ЯАГААД БОЛОХГҮЙ ВЭ:
 *
 * `uploadId` бол зүгээр нэг зам биш — `/api/payment` дээр ЭНЭ НЬ ӨӨРӨӨ
 * нэвтрэлт болдог (тэндхийн тайлбарыг үз). Захиалагч линкээ хуваалцахад
 * хангалттай, дугаараа таасан хүн орж чадахгүй байх ёстой цорын ганц нууц.
 *
 * `Math.random` нь V8 дотор xorshift128+ ашигладаг бөгөөд гаралтуудыг нь
 * хангалттай ажигласан хүн дотоод төлөвийг нь сэргээж, ӨМНӨХ болон ДАРААХ
 * утгуудыг тооцоолж чаддаг. Өөрөөр хэлбэл халдагч өөрөө хэдэн захиалга
 * өгөөд, өөрийн авсан id-уудаас бусад хүний id-г гаргах онолын зам үлдэнэ.
 * 2^80 гэсэн орон зай нь СОХОР таахаас хамгаална, төлөв сэргээхээс биш.
 *
 * `crypto.getRandomValues` нь ийм төлөвгүй. Vercel-ийн edge орчинд стандартаар
 * байдаг тул нэмэлт сан ч шаардлагагүй.
 */
const cryptoRandom = (): number => {
  const buffer = new Uint32Array(1);
  crypto.getRandomValues(buffer);
  // 2^32-т хуваахад `[0, 1)` завсар гарна — `Math.random`-той ижил гэрээ.
 ...
 *
 * Цагаан толгой нь 32 тэмдэгт — ХОЁРЫН ЗЭРЭГТ. Тиймээс `floor(random() * 32)`
 * нь модулийн хазайлтгүй: тэмдэгт бүр яг 5 бит авна, 16 тэмдэгт = 80 бит.
 * (`l`, `o`, `0`, `1` байхгүй — утсаар уншиж бичихэд андуурдаг.)
 *
 * `random`-ыг параметр болгосон нь зөвхөн ТЕСТэд зориулсан. Ажлын үед
 * өгөгдмөл нь үргэлж криптографик эх сурвалж байх ёстой.
 */
export const makeUploadId = (random: () => number = cryptoRandom): string => {
...
/**
 * `PMN-260806-48213` хэлбэрийг л зөвшөөрнө.
 *
 * ⚠️ `\d{4,5}` — ЗӨВХӨН `\d{5}` БИШ. Шинэ дугаар таван оронтой боловч R2 дээр
 * аль хэдийн хадгалагдсан ДӨРВӨН оронтой захиалгууд байгаа. Зөвхөн таван
 * оронг зөвшөөрвөл тэдгээр хуучин захиалгын төлөв харах хуудас, NAS-ийн
 * таталт, түлхүүр задлагч гурвуулаа тэр дороо ажиллахаа болино — өөрөөр
 * хэлбэл өнгөрсөн бүх захиалга хүрэлцэхгүй болно.
 */
export const isOrderNumber = (value: string): boolean =>
  /^PMN-\d{6}-\d{4,5}$/.test(value);
