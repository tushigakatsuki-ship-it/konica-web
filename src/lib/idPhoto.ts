 ...
 *
 * Автомат тайралт нь илрүүлсэн ТОЛГОЙН ӨНДРӨӨС тооцогддог. Илрүүлэлт
 * бага зэрэг алдвал (үс өндөр, эрүү бүдэг) хүрээ хэт жижиг гарч, хүн
 * зурагт хэт том орно.
 *
 * Дахин зураг авахуулах нь үнэтэй. Гулсуураар засах нь секундын ажил.
 * ────────────────────────────────────────────────────────────────── */

/** Томруулалтын хязгаар. 1 = автомат тооцоолсон хэмжээ. */
export const ZOOM = { min: 0.6, max: 2, step: 0.01, default: 1 } as const;

/**
 * Хүрээг ТӨВӨӨР нь томруулж, зургийн дотор багтаана.
 *
 * Байрлалыг `x/y` биш ТӨВӨӨР нь илэрхийлсэн нь санаатай: томруулахад төв
 * тогтвортой үлдэх ёстой. `x/y`-аар бодвол томруулах бүрд зураг гулсана.
 *
 * `zoom` их = хүрээ жижиг = хүн ТОМ харагдана.
 */
export function zoomCrop(
  ...
  // Зурагнаас хэтэрвэл ХАРЬЦАА хадгалж багасгана — эс бөгөөс сунана.
  const fit = Math.min(1, imageWidth / w, imageHeight / h);
  w *= fit;
  h *= fit;

  return {
    x: Math.max(0, Math.min(imageWidth - w, center.x - w / 2)),
    y: Math.max(0, Math.min(imageHeight - h, center.y - h / 2)),
    w,
    h,
  };
}

/** Хүрээн дэх толгойн эзлэх хувь — стандарт яг үүгээр хэмждэг. */
export const headRatioIn = (faceHeight: number, cropHeight: number): number =>
  cropHeight > 0 ? faceHeight / cropHeight : 0;

/**
 * Стандартаас хазайхыг зөвшөөрөх хэмжээ.
 *
 * Ажилтан гараар томруулах эрхтэй — гэхдээ стандартаас хэт хазайвал зураг
 * буцаагдана. Хориглохгүй, СЭРЭМЖЛҮҮЛНЭ: заримдаа зөв шалтгаанаар (өндөр
 * үс, малгай) хазайлгах шаардлага гардаг.
 */
export const HEAD_RATIO_TOLERANCE = 0.06;
