 ...
 * `PhotoEditor`-т АВТОМАТ нүүр илрүүлэлт, автомат тайралт, `isPrintReady`
 * маягийн хаалт хэзээ ч сэргээгдээгүй. Шийдэл нь ХААЛТТАЙ компонентыг
 * (`IdPhotoOrder`) энд авчрах явдал.
 *
 * ⚠️ 2026 оны 8-р сараас (`src/lib/adjust.ts`, `src/lib/backdrop.ts`)
 * `CropStudio` дотор `idPhoto` prop-той үед тодруулах/blur/тодотгох/дэвсгэр
 * солих жижиг хэрэгсэл нэмэгдсэн — гэхдээ ЭНЭ БОЛ автомат нүүр илрүүлэлт
 * БИШ: дэвсгэр солих сонголт ЗӨВХӨН `fitBackdrop`-ийн жигд гэж баталгаажуулсан
 * зурагт л идэвхжинэ, эмх замбараагүй бол идэвхгүй + тайлбартай харагдана.
 */
...
/**
 * Цээж зургийн засварын хэрэгсэл (brightness/blur/sharpen/дэвсгэр).
 *
 * Гурван зүйлийг шалгана: (1) шинэ UI ЗӨВХӨН `idPhoto` үед унших/зурагддаг,
 * бусад 11 ангилалд огт хамааралгүй; (2) дэвсгэрийн сонголт эмх замбараагүй
 * үед CSS-ээр бүдгэрүүлж ХАРУУЛСАН ХЭВЭЭР биш, чухам `disabled` атрибуттай
 * болдог; (3) `idPhoto` нь тухайн мөчийн `tab`-аас биш, засварлаж буй
 * `service.category`-аас тооцогддог (сагснаас «Засах»-аар нээхэд ч зөв байх).
 */
test('Цээж зургийн засварын UI зөвхөн idPhoto үед харагдана', () => {
  const cropStudio = readCode('src/components/CropStudio.tsx');

  assert.match(cropStudio, /\{idPhoto && \(/, 'шинэ хяналт idPhoto-оор хамгаалагдаагүй');
  assert.ok(
    cropStudio.includes("disabled={backdropEligible !== true}"),
    'дэвсгэрийн товч жинхэнэ disabled БИШ (зөвхөн CSS-ээр далдлагдсан байж болзошгүй)',
  );
  assert.ok(
    cropStudio.includes("t('adjust.bgUnavailable')"),
    'эмх замбараагүй дэвсгэрт тайлбар алга — хэрэглэгч юу болсныг мэдэхгүй',
  );

  const photoEditor = readCode('src/components/PhotoEditor.tsx');
  assert.ok(photoEditor.includes('idPhoto={idPhoto}'), 'idPhoto CropStudio руу дамжаагүй');

  const print = readCode('src/pages/Print.tsx');
  assert.ok(
    print.includes("idPhoto={editorFor.service.category === 'Цээж зураг'}"),
    'idPhoto нь service.category-аас БИШ, tab-аас тооцогдож байна — «Засах»-аар нээхэд буруу байж болзошгүй',
  );
});
...
test('дэвсгэр зураг солигдох нь хүртээмж, гүйцэтгэлийг зөрчөөгүй', () => {
  ...
  /*
   * ⚠️ Гүн буланг ЯГ `/85` гэж түгжихээ болив — тэр нь зорилгоо давсан хатуу
   * шалгалт байв: халхавчийг ЧАНГАРУУЛАХАД ч тест уначихдаг байлаа. Хамгаалах
   * ёстой зүйл нь тодорхой тоо биш, харин «гүн булан сулраагүй» гэдэг — тиймээс
   * доод хязгаараар шалгана.
   */
  const deep = heroCode.match(/from-brand-500\/(\d+)/);
  assert.ok(deep, 'халхавчийн гүн хэсэг алга');
  assert.ok(
    Number(deep[1]) >= 85,
    `халхавчийн гүн хэсэг сулрсан: ${deep[1]}% (≥85 байх ёстой)`,
  );

  /*
   * Дэвсгэрийн ӨНГӨ логотой таарах механизм.
   *
   * Тунгалаг халхавч дангаараа логоны өнгийг барьж ЧАДДАГГҮЙ — доорх зурагтай
   * холилдож цайрдаг (хэмжсэн: #1a56db → #768ebd). `mix-blend-color` нь өнгийг
   * тонгоруулгын давхаргаас, гэрлийг зурагнаас авдаг тул логоны токентой таарна.
   *
   * Гурвуулаа хамт байх ёстой:
   *   `bg-brand-500`      — өнгө хаанаас ирэх (логоны хавтантай ижил токен)
   *   `mix-blend-color`   — өнгө барих механизм
   *   `isolate`           — холилт hero-гоос гадагш гарахаас сэргийлнэ
   */
  for (const need of ['bg-brand-500', 'mix-blend-color', 'isolate']) {
    assert.ok(
      heroCode.includes(need),
      `дэвсгэрийн өнгө логотой таарах механизм эвдэрсэн — «${need}» алга`,
    );
  }

  /*
   * ⚠️ Тонгоруулга БҮТЭН байж болохгүй.
   *
   * `opacity-100` бол зураг бүрэн монохром болж, хэт хатуу, нүд ядраам
   * харагдана — хэрэглэгчийн гомдол яг үүн дээр гарсан. Зургийн жинхэнэ өнгө
   * хэсэгчлэн үлдэх ёстой.
   */
  const tint = heroCode.match(/bg-brand-500 opacity-(\d+) mix-blend-color/);
  assert.ok(tint, 'тонгоруулгын давхарга алга');
  assert.ok(
    Number(tint[1]) <= 60,
    `тонгоруулга хэт бүтэн: ${tint[1]}% (≤60 байх ёстой — эс бөгөөс монохром болно)`,
  );
...
/* ── Гэрэл горимыг АЛБАДАНА ───────────────────────────────────── */

test('өгөгдмөл горим нь ГЭРЭЛ — зургийн өнгө үнэн харагдах ёстой', () => {
  /*
   * ⚠️ Энэ бол зан сонголт биш, зургийн бизнесийн шаардлага.
   *
   * Хэрэглэгч оруулсан зургаа дэлгэц дээр хараад «ийм өнгөтэй хэвлэгдэнэ»
   * гэж шийддэг. Харанхуй дэвсгэр дээр зураг илүү тод, өнгөлөг харагддаг
   * тул цаасан дээр буухдаа бүдэг санагдана. Фото засварын програм бүр
   * саарал/цагаан дэвсгэр хэрэглэдэг нь яг үүнээс.
   */
  const theme = readCode('src/state/theme.ts');
  assert.match(theme, /DEFAULT_CHOICE: ThemeChoice = 'light'/, 'өгөгдмөл нь гэрэл биш байна');
  assert.ok(
    !/: ThemeChoice = 'system'/.test(theme),
    'өгөгдмөл `system` рүү буцсан — үйлдлийн систем харанхуй бол зураг худал өнгөтэй харагдана',
  );
});

test('index.html ба theme.ts ижил өгөгдмөлтэй — анивчихаас сэргийлнэ', () => {
  /*
   * Хоёр газар өгөгдмөлөө барьдаг: `index.html` доторх блоклодог скрипт
   * (React ачаалагдахаас өмнө) ба `theme.ts` (түүний дараа). Зөрвөл хуудас
   * эхлээд нэг өнгө гараад дараа нь нөгөө рүү үсэрч анивчина.
   *
   * Telegram-ийн товч яг ийм «хоёр газар тусад нь мэддэг» алдаанаас
   * эвдэрсэн тул энд тестээр холбов.
   */
  const html = read('index.html');
  assert.match(html, /\? t : 'light'/, 'index.html нь гэрэл рүү унадаггүй байна');
  assert.match(html, /catch \(e\) \{\s*document\.documentElement\.setAttribute\('data-theme', 'light'\)/,
    'localStorage унасан үед горим тодорхойгүй үлдэж байна');
});

test('color-scheme мета нь гэрэл гэж зарлана', () => {
  /*
   * `content="light dark"` үлдвэл браузерын өөрийн хэсэг (гүйлгэх мөр,
   * хайлтын талбар) харанхуй хэвээр гарч, хуудастай зөрнө.
   */
  const html = read('index.html');
  assert.match(html, /name="color-scheme" content="light"/);
  assert.ok(
    !/prefers-color-scheme: dark"\s*\/>/.test(html),
    'theme-color дээр харанхуйн хувилбар үлдсэн байна',
  );
});

test('эхлэх хэл нь ҮРГЭЛЖ монгол — хөтчийн хэлээр таамаглахгүй', () => {
  /*
   * Улаанбаатарт маш олон монгол хүн утсаа англи хэл дээр тохируулж
   * хэрэглэдэг. `navigator.language`-аар таамаглавал тэдэнд монгол
   * дэлгүүрийн сайт англиар нээгдэнэ — ашиггүй, зарим нь «буруу сайт»
   * гэж бодоод гарна.
   *
   * Толгойд MN/EN товч байнга харагддаг тул англиар үзэх хүн нэг дарахад
   * л болно. Таамаглал нь ашгаас илүү эрсдэл авчирдаг.
   */
  const lang = readCode('src/state/lang.tsx');
  assert.ok(
    !/navigator\.language/.test(lang),
