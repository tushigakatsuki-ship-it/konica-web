...
test('токены УРТЫГ ч хэлэхгүй', async () => {
  /*
   * Урт нь өөрөө таах ажлыг хөнгөвчилдөг тул хариунд огт орох ёсгүй.
   *
   * ⚠️ Энэ шалгалтыг эхлээд «хариу бүхэлдээ уртын тоог агуулаагүй» гэж бичсэн
   * нь ТОГТВОРГҮЙ байсан: хуурамч серверийн ПОРТ санамсаргүй тул `storage`
   * тайлбар доторх `127.0.0.1:2812` мэт хаяг тэр тоог санамсаргүй агуулаад
   * тест хааяа унадаг байв. Тиймээс токенд хамаарах талбарыг л шалгана.
   */
  configure();
  const body = await (await call()).json();

  assert.ok(
    !/\d/.test(body.checks.admin.detail),
    `admin тайлбарт тоо байх ёсгүй: ${body.checks.admin.detail}`,
  );
  assert.equal(body.checks.admin.missing, undefined, 'бэлэн үед нэр ч гарахгүй');
});
